              ...
              <div class="row">
                ...
                <div class="col-md-6">
                  ...
                  <a href="<?= base_url('master/User/create') ?>">
                    <button type="button" class="btn btn-sm btn-success"><i class="fa fa-plus"></i> Tambah User</button> 
                  </a>
                  <a href="<?= base_url('fitur/ekspor/user') ?>" target="_blank">
                    <button type="button" class="btn btn-sm btn-warning"><i class="fa fa-file-excel-o"></i> Ekspor User</button> 
                  </a>
                  <button type="button" class="btn btn-sm btn-info" onclick="$('#modal-impor').modal()"><i class="fa fa-file-excel-o"></i> Import User</button>
                  </div>
                </div>  
              </div>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Impor User</h4>
        </div>
  ...
  <script type="text/javascript">
        ...
        function loadtable(status) {
            var table = '<table class="table table-bordered" id="mytable">'+
                   '     <thead>'+
                   '     <tr>'+
                   '       <th style="width:20px">No</th>'+'<th>Nip</th>'+'<th>Name</th>'+'<th>Email</th>'+'<th>Password</th>'+'<th>Role Id</th>'+'<th>Desc</th>'+'       <th style="width:150px">Status</th>'+
             ...
             $("#load-table").html(table)
              ...
              var t = $("#mytable").dataTable({
                ...
                columns: [
                    {"data": "id","orderable": false},{"data": "nip"},{"data": "name"},{"data": "email"},{"data": "password"},{"data": "role_id"},{"data": "desc"},
                   {"data": "status"},
                    {   "data": "view",
                        "orderable": false
                    }
                    ...
                    { targets : [7],
                        render : function (data, type, row, meta) {
                              if(row['status']=='ENABLE'){
                                var htmls = '<a href="<?= base_url('master/User/status/') ?>'+row['id']+'/DISABLE">'+
                                            '    <button type="button" class="btn btn-sm btn-sm btn-success"><i class="fa fa-home"></i> ENABLE</button>'+
                                            '</a>';
                              }else{
                                var htmls = '<a href="<?= base_url('master/User/status/') ?>'+row['id']+'/ENABLE">'+
