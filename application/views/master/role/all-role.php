 ...
 <!-- Content Wrapper. Contains page content -->
  ...
  <script type="text/javascript">
        ...
        function loadtable(status) {
            var table = '<table class="table table-bordered" id="mytable">'+
             ...
             $("#load-table").html(table)
              ...
              var t = $("#mytable").dataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: {"url": "<?= base_url('master/role/json?status=') ?>"+status, "type": "POST"},
                columns: [
                    ...
                    { targets : [2],
                        render : function (data, type, row, meta) {
                              if(row['status']=='ENABLE'){
                                var htmls = '<a href="<?= base_url('master/role/status/') ?>'+row['id']+'/DISABLE">'+
                                            '    <button type="button" class="btn btn-sm btn-sm btn-success"><i class="fa fa-home"></i> ENABLE</button>'+
                                            '</a>';
                              }else{
                                var htmls = '<a href="<?= base_url('master/role/status/') ?>'+row['id']+'/ENABLE">'+
         ...
         function edit(id) {
            location.href = "<?= base_url('master/role/edit/') ?>"+id;
         }
