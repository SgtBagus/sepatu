...
Selamat siang, Bedasarkan hasil diskusi NBS dengan Mas Hafiz pada tanggal 05 Maret 2019 di PLN Assesment Center tentang integrasi API MK - DAC PLN, ada beberapa point yang perlu disesuaikan di format API yang mas share ke kita. Beberapa point yang perlu disesuaikan dari API yg dibangun mas Hafiz adalah

1. Format API yang diberikan masih belum sesuai dengan format standard API secara umum, untuk contohnya bisa menggunakan format yang saya lampirkan pada tautan ini Doc API Example. Format ini dibangun dari referensi yang kami gunakan pada tautan JSend ini dengan menyesuaikan kebutuhan secara umum.
2. Kami menemukan beberapa response return-nya belum konsisten dan tidak deksriptif
Contoh
- data salah parameter dengan return []
  {
    "status": false,
    "data": {
      "id": 53,
...
- respon dengan return [" No Jadwal were found"]
  {
    "status": false,
    "data": {
      "id": 43
    ...
    "response": [
      "No jadwal were found"
    ]
  }
- data yang berhasil login dengan return [true]
  {
    "status": true,
    "data": {
      "nip": "nip-123124"
...
Hal ini akan menyulitkan di sisi kami untuk mentrace problem dikemudian hari jika terjadi masalah selama integrasi berjalan.

Beberapa concern kami adalah
A. Akan kesuliltan untuk memastikan respon untuk data tidak ditemukan atau data yg dikirim parameternya salah,karena dari sis API mas hanya mengembalikan respon yang tidak konsisten dan jelas untuk berbagai kondisi
B. Memastikan respon API dengan lebih deksriptif
status_code 200 success, ini artinya api yang dikirim parameter dan endpoint-nya berhasil dan mengembalikan respon
status_code 401 Unauthorized, ini artinya user belom ter-autorisasi (bisa belum login/belum menyertakan token) pada sistem
status_code 404 data not found, ini artinya data yang kami cari tidak ditemukan
status_code 400 Bad Request, ini artinya ada yang salah dengan pengiriman parameter dari client side
status_code 500 Internal Server Error, ini artinya ada kesalahan (catch error) pada server
untuk status code lebih lengkap bisa di cek disini List Status Code API
C.tidak ada informasi type data di setiap proses transaksi API,kami mohon ini dibantu diprovide

3. Belum ada gambaran yang cukup jelas jika terdapat API error yg dapat ataupun tidak dapat dihandle di layer aplikasi, mas bisa menggunakan link postman kita yang diatas untuk mencoba mendapat gambaran 

Kami sangat menyarankan menghindari deskriptif respon hanya dengan array true/false atau dengan array string, karena tidak deksriptif dan sulit ditrace jika terjadi masalah pada parameter atau return yang diterima dari respon.
Seperti yang kami sampaikan diawal mohon bantuannya untuk membangun dokumentasi API dengan sample diatas
